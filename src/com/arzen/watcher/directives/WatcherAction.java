 ...
 * 守望者父类 
 * 执行的操作指令： 
 * 1.指令的格式：指令关键字 应用范围 指令动作 指令参数1指令参数
 * 2.指令关键字:shortcut, banner,msg,mark,info 
 * 应用范围:all为应用所有情况， KEY1,KEY2仅允许符合指定的KEY才执行相应的指令，多个KEY以逗号隔开。
 * 指令动作：依据不同的指令，有不同的动作。
...
public abstract class WatcherAction {
	...
	/**
	 * 解析收到的指令
	 * 1.指令的格式：指令关键字 应用范围 指令动作 指令参数1,指令参数
	 * @param directivesStr 需要解析的指令字符串
	 * @return
	 */
